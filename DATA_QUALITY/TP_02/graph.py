...
def get_plot_from_dataframe(months, climat, mean, std, minli, maxli, minyear, maxyear):
    ...
    suptitle = plt.suptitle(
        "\n\nMoyenne: " + str(round(mean[month], 2)) + ", écart-type: " + str(round(std[month], 2)) + " min: " + str(
            minli[month]) + " max: " + str(round(maxli[month],2)), fontsize=10)
    plt.xlabel('Jours')
    ...
    class Index:
        ...
        def prev(self, event):
            if self.ind > 0:
                self.ind -= 1
                title.set_text(f"Température du mois de {months[self.ind]} \n ")
                suptitle = plt.suptitle("\n\nMoyenne: " + str(round(mean[self.ind])) + ", écart-type: " + str(
                    std[self.ind]) + " min: " + str(minli[self.ind]) + " max: " + str(maxli[self.ind]), fontsize=10)
                plot.set_ydata(climat[:, self.ind])
                self.cursor.x = arange(len(climat[:, self.ind]))
                self.cursor.y = climat[:, self.ind]
                if self.ind == 11:
